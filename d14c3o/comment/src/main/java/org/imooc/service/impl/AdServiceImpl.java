...
public class AdServiceImpl implements AdService {
	...
	private String adImageSavePath;  //配置上传文件的路径 广告图片保存路径
	...
	public List<AdDto> searchByPage(AdDto adDto) {
		...
		Ad condition = new Ad();
		BeanUtils.copyProperties(adDto, condition);  //TODO Bean拷贝 两个对象之间属性名相同就把值赋值过去  上面那个set方法就是手动实现的
	...
	public boolean modify(AdDto adDto) {
		...
		int updateCount = adDao.update(ad);  //根据影响的条数来判断
		if (updateCount != 1) {
			return false;
		}
		if (fileName != null) {
